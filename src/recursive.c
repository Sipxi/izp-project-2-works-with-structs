...
int findLongestLine(Bitmap *bitmap, int start_index[], 
                    ...
                    int *end_pointer)
{
    if (bitmap->data[start_index[0]][start_index[1]] == 0);
    printf("yes, it's 0, index %d, %d", start_index[0], start_index[1]);
}
...
void findLongestLineBitmap(Bitmap *bitmap, int *start_position, int *end_position, bool search_horizonally)
{
    ...
    findLongestLine(bitmap, start_index, end_index, search_horizonally, start_position, end_position);
}
